...
def main():
    ...
    # Initialize the grid grid, source node, and goal node.
    gridworld = WeightedGridworld(GRID_SIZE[0]//SQUARE_SIZE, GRID_SIZE[1]//SQUARE_SIZE)
    gridworld.weights = {(x, y): 1 for x in range(gridworld.width) for y in range(gridworld.height)}
    ...
    while True:
        ...
        for event in pg.event.get():
            ...
            # source, goal, or obstacle being dragged into new position; update position accordingly.
                ...
                if event.user_type == pygame_gui.UI_DROP_DOWN_MENU_CHANGED:
                    ...
                    if event.ui_element == start_button:
                        ...
                        gridworld = WeightedGridworld(GRID_SIZE[0]//SQUARE_SIZE, GRID_SIZE[1]//SQUARE_SIZE)
                        gridworld.weights = {(x, y): 1 for x in range(gridworld.width)
                                                    for y in range(gridworld.height)}
        ...
        # Set up the screen, background, and the GUI.
        screen.fill(pg.Color('grey'))
        background.fill(pg.Color('grey'))
        manager.draw_ui(screen)

        # Actually draw the grid with source, node, and obstacles onto the screen.
        ...
        if start:
            ...
            if algo_chosen == "DFS":
                ...
                visited, cost_dijkstra = dijkstra(gridworld, gridworld.source, gridworld.goal)
            elif algo_chosen == "A*":
                visited, cost_a_star = a_star(gridworld, gridworld.source, gridworld.goal)
